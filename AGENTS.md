...
| `tests/` | **Golden-set regression** over the executable governance artefacts — 340 assertions across eight suites, no token cost. `just test`; gated by `just check`. Hermetic except the model-pin check, which reads the live OpenRouter catalog. That check, the resolved-permission suite and the context7 server check need the `opencode` binary and skip without it — 282 assertions run in CI, 340 locally. A skip is counted apart from a pass and named at the end of the run; `just check` refuses a suite that asserted nothing, while `just test` still exits 0 so CI is not forced into an install decision (AIOPS-24). |
