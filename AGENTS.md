...
| `.agents/` | Harness-neutral artefact root; `policy` is a symlink → `../policy`. `evals/` is a real directory — nothing distributes it, so there is no source-and-copy split to keep honest. |
| `.agents/evals/` | **Agent-output evals** — the half `tests/` cannot cover, since the golden set never asks a model anything. `just eval`; **not** part of `just check`, because a gate that spends money per run is one people skip. Each agent runs on the model its own frontmatter pins; a run prints its cost and aborts above the budget. The three decisions behind it: `.agents/evals/README.md` |
...
| `tests/` | **Golden-set regression** over the executable governance artefacts — 357 assertions across eight suites, no token cost. `just test`; gated by `just check`. Hermetic except the model-pin check, which reads the live OpenRouter catalog. That check, the resolved-permission suite and the context7 server check need the `opencode` binary and skip without it — 299 assertions run in CI, 357 locally. A skip is counted apart from a pass and named at the end of the run; `just check` refuses a suite that asserted nothing, while `just test` still exits 0 so CI is not forced into an install decision (AIOPS-24). |
